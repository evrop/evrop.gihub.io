...
function initialize() {
    for (var i = 0; i < this.n; ++i) {
        ...
        this.b_vector[i] = funk.value;
    }
}

function get_matrix_element(i, j, funk) {
    if (Math.abs(i - j) < 1.01) {
        this.A_matrix[i][j] = this.int_cells(i, j, function (x) {
...
function draw_plot() {
    ...
    var board = JXG.JSXGraph.initBoard('jxgbox', {keepaspectratio: true, boundingbox: [-5, 1, 1, -5], axis: true});
    var x = this.a;
...
function ch(){
    /*for(var i = 1; i < this.A_matrix[0].length-1; ++i){
        this.A_matrix[0][i] -= this.diff_fi_i(0, this.a) * this.f_a;
        this.A_matrix[i][0] -= this.diff_fi_i(0, this.a) * this.f_a;
        this.A_matrix[this.A_matrix[0].length-1][i] += this.diff_fi_i(this.n - 1, this.b) * this.f_b;
        this.A_matrix[i][this.A_matrix[0].length-1] += this.diff_fi_i(this.n - 1, this.b) * this.f_b;
    }
    this.A_matrix[0][0] -= this.diff_fi_i(0, this.a) * this.f_a;
    this.A_matrix[this.A_matrix[0].length-1][this.A_matrix[0].length-1] += this.diff_fi_i(this.n - 1, this.b) * this.f_b;
